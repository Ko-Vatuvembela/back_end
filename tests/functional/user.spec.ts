...
test.group('Testar a funcionalidades relacionadas ao utilizador', (group) => {
  ...
  test('Eliminar os dados de um utilizador existente', async ({ client }) => {
    let utilizador: UserType = createFakeUser();
    const request = await client.post('/api/user').json(utilizador);
    const uid = request.response.body.uid;
    const response = await client.delete(`/api/user/${uid}`);
    response.assertStatus(200);
  });

  test('Eliminar os dados de um utilizador não existente - retorna 404', async ({ client }) => {
    const response = await client.delete(`/api/user/${v4()}`);
    response.assertStatus(404);
  });
});
